  ...
  private matchesAttributeFilter(
    ...
    if (!attribute) {
      ...
      if (!("$ne" in filterValue)) {
        /**
         * Allows nil attribute for $ne operation
         */
        return false;
      }
    }
    ...
    switch (operator) {
      ...
      case "$ne": {
        ...
        if (Array.isArray(targetValue)) {
          const filterValueArr = String(filterValue).split("\n");

          return !intersection(targetValue, filterValueArr).length;
        }
        return targetValue !== filterValue;
      }
      ...
      case "$in": {
        if (Array.isArray(targetValue) && Array.isArray(filterValue)) {
          return intersection(targetValue, filterValue).length > 0;
        }
